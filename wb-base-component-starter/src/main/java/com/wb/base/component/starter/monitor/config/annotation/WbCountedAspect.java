...
import com.wb.base.component.starter.monitor.utils.ExpressionEvaluator;
...
public class WbCountedAspect {
     ...
     */
    public WbCountedAspect(MeterRegistry registry) {
     ...
     */
    public WbCountedAspect(MeterRegistry registry, Function<ProceedingJoinPoint, Iterable<Tag>> tagsBasedOnJoinPoint) {
     ...
     * @since 1.7.0
     */
    public WbCountedAspect(MeterRegistry registry, Predicate<ProceedingJoinPoint> shouldSkip) {
        this(
                registry,
     ...
     * @since 1.7.0
     */
    public WbCountedAspect(MeterRegistry registry, Function<ProceedingJoinPoint, Iterable<Tag>> tagsBasedOnJoinPoint, Predicate<ProceedingJoinPoint> shouldSkip) {
    ...
    public Object interceptAndRecord(ProceedingJoinPoint pjp, Counted counted) throws Throwable {
        ...
        if (stopWhenCompleted) {
            try {
                ...
                record(pjp, counted, e.getClass().getSimpleName(), RESULT_TAG_FAILURE_VALUE, ExceptionTagUtils.buildExceptionCode(e).getValue());
                throw e;
            }
        }
        ...
        try {
            ...
            if (!counted.recordFailuresOnly()) {
                record(pjp, counted, DEFAULT_EXCEPTION_TAG_VALUE, RESULT_TAG_SUCCESS_VALUE, ExceptionTagUtils.SUCCESS_CODE);
            }
            return result;
        } catch (Throwable e) {
            record(pjp, counted, e.getClass().getSimpleName(), RESULT_TAG_FAILURE_VALUE, ExceptionTagUtils.buildExceptionCode(e).getValue());
    ...
    private void recordCompletionResult(ProceedingJoinPoint pjp, Counted counted, Throwable throwable) {
        ...
        if (throwable != null) {
            ...
            record(pjp, counted, exceptionTagValue, RESULT_TAG_FAILURE_VALUE, ExceptionTagUtils.buildExceptionCode(throwable).getValue());
    ...
    private void record(ProceedingJoinPoint pjp, Counted counted, String exception, String result, String code) {
        String[] tags = new String[counted.extraTags().length];
        if (null != counted.extraTags()){
            for (int i = 0; i < counted.extraTags().length; i++) {
                tags[i] = ExpressionEvaluator.getConditionValue(pjp, counted.extraTags()[i], String.class);
            }
        }
        counter(pjp, counted)
    ...
    private Counter.Builder counter(ProceedingJoinPoint pjp, Counted counted) {
        Counter.Builder builder = Counter.builder(MonitorKeyUtils.buildMonitoryKey(counted.value(), Meter.Type.COUNTER)).tags(tagsBasedOnJoinPoint.apply(pjp));
